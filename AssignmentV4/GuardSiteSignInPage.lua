...
-- Create table for initial object positions
	...
	if readMeText ~= "" then
		...
		infoBoxBack:setFillColor( 1,1,0.6 ) 
		...
		local function controlInfoBox( event )
			if event.phase == "began" then
				if infoBoxState == "canOpen" then
					...
					transition.to( textGroupContainer, { time=900, tag="infoBox", y=objPos["infoBoxDestY"] } )
					transition.to( textGroupContainer, { time=400, tag="infoBox", delay=750, xScale=1, yScale=1, onComplete=transComplete } )
				...
				elseif infoBoxState == "canClose" then
					...
					transition.to( textGroupContainer, { time=400, tag="infoBox", xScale=0.96, yScale=0.96 } )
					transition.to( textGroupContainer, { time=700, tag="infoBox", delay=200, y=objPos["infoBoxOffY"], onComplete=transComplete } )
				end
			end
			return true
		end
