...
local function reloadView()
	...
	for row in db:nrows("SELECT * FROM site") do
		...
		tableView:insertRow({rowHeight = 110, params = rowParams,})
	end		
end
...
local function deleteScene(event)
	if ( "ended" == event.phase ) then
		-- Delete from the site table in the database.
...
local function onRowRender(event)
	...
	row.deleteButton.y = rowHeight +30
...
local function AddScene( event )
	if ( "ended" == event.phase ) then
		-- Go to Add Site Page.
		composer.gotoScene("AddSitePage",{effect = "fromRight",time = 500})		
	end		
end
