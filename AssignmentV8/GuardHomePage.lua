	...
	-- Else no errors
		...
		for row in db:nrows("SELECT * FROM site") do
			if row.siteLatitude < '0' then
				minLat = tonumber(row.siteLatitude + 0.0100)
				maxLat = tonumber(row.siteLatitude - 0.0100)
			else
				minLat = tonumber(row.siteLatitude - 0.0100)
				maxLat = tonumber(row.siteLatitude + 0.0100)
			end
			if row.siteLongitude < '0' then
				minLong = tonumber(row.siteLongitude + 0.0100)
				maxLong = tonumber(row.siteLongitude - 0.0100)
			else
				minLong = tonumber(row.siteLongitude - 0.0100)
				maxLong = tonumber(row.siteLongitude + 0.0100)
			end
			...
			if ( (currLatitude>= minLat and currLatitude<= maxLat) and (currLongitude >= minLong and currLongitude <= maxLong))then 
