...
-- Functions.
------------------------------------------------------------------
-- If the user selects the 'Finish Shift' button.
local function ShiftScene( event )
	if ( "ended" == event.phase ) then
		-- Go to Guard Finish Shift Page
		composer.gotoScene("GuardFinishShiftPage",{effect = "fromRight",time = 500})		
	end	
end
...
-- If the user selects the 'SOPs' button.
local function SOPScene( event )
	if ( "ended" == event.phase ) then
		-- Go to SOP Details.
		composer.gotoScene("SOPDetails",{effect = "slideUp",time = 800})		
	end	
end

-- If the user selects the 'Post Order' button.
local function PostOrderScene( event )
	if ( "ended" == event.phase ) then
		-- Go to Post Order Details.
		composer.gotoScene("PostOrderDetails",{effect = "slideUp",time = 800})		
	end	
end
...
local function EmergencyScene( event )
	...
	if ( buttonID == "" ) then
		-- Create native alert
		local alertBox = native.showAlert( "Emergency Contact", "Please call this number 0123456789!", { "Cancel", "Contact Security Team" },
		function( event )
