...
-- Function to make sure the name is in the database.
local function nameInDataBase(str)
	-- Loop through the employee table and return true if name is found.
	for row in db:nrows("SELECT * FROM employee") do
		if (row.empName == str) then
			return true
		end
	end
	-- Return false if the name is not found in the database.
	return false
end
...
local function inputValidation()
	...
	-- If date is the correct format, check if the name is in the database and return 1 if true.
	elseif (isValidDate(dateField.text)) then
		if (nameInDataBase(nameField.text)) then
			return 1
		-- If the name is not found in the database, show alert message and return 0.
		else
			local alert = native.showAlert( "Invalid input", "Name not found in database.", { "OK" } )
			return 0
		end

	-- If input is not valid, show alert and return 0.
	else
		local alert = native.showAlert( "Invalid input", "Date must be in the correct format 'dd/mm/yyyy'", { "OK" } )
		return 0
	end
end
